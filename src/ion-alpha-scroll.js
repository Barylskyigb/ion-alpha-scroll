angular.module("ion-alpha-scroll", []).directive("ionAlphaScroll", [
  "$ionicScrollDelegate",
  "$location",
  "$timeout",
  "$document",
      ...
      compile: function(tElement, tAttrs, tTransclude) {
        ...
        var template = angular.element(
          [
            ...
            '<ion-item ng-repeat="item in items" ng-class="{first: $first, last: $last}"></ion-item>',
            "</div>",
            "</ion-scroll>",
            '<ul class="ion_alpha_sidebar">',
            '<li ng-click="alphaScrollGoToList(\'index_{{letter}}\')" ng-repeat="letter in sortedAlphabet">{{letter}}</li>',
            "</ul>",
            "</ion-list>"
          ].join("")
        );

        var headerHeight = $document[0].body.querySelector(".bar-header")
          .offsetHeight;
        var subHeaderHeight = tAttrs.subheader === "true" ? 44 : 0;
        var tabHeight = $document[0].body.querySelector(".tab-nav")
          ? $document[0].body.querySelector(".tab-nav").offsetHeight
          : 0;
        var windowHeight = window.innerHeight;

        var contentHeight =
          windowHeight - headerHeight - subHeaderHeight - tabHeight;
        ...
        return function(scope, element, attrs, ngModel) {
          ...
          ngModel.$render = function() {
            ...
            scope.sortedAlphabet = Object.keys(tmp).sort();
